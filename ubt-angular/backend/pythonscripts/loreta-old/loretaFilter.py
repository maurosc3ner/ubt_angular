#!/usr/bin/env python
#Plot3DBrain
# python loretaFilter.py  -l nyL.mat -v nyvert.mat -fnyface.mat -c Channels.mat -d EEG_data.txt -b ..\..\server_data\sujeto_base.edf
#python Main_BM.py -l nyL.mat -v nyvert.mat -f nyface.mat -c nyCh.mat -d nyEEG_R.txt -b sujeto_base.edf
...
l=Main_L() #Cargar la funciones necesarias para la ejecucion del programa
path = './' #
lead,verts,faces,channels,data,base =l.argparse() #Direccion y nombre de archivos (Lead_field,verts,faces,channels,EEG.txt,sujetobase.edf)
...
for i in range(0,np.shape(J_energy)[0]):
        if J_energy[i]<0.2:
            J_energy[i] = 0.0
print('dimesiones fuentes ',np.shape(J_energy))#Mostrar dimensiones de las fuentes
l.toc()# Final del contador
colors = l.IntensitytoJet(J_energy)
colorExporter=colors[:,[0,1,2]]/255 # Seleccionar las tres primeras columnas y normalizar los valores del color 
color={}
color['items']=[]
#Exportar matriz de color y energia
for i in range(0,np.shape(colors)[0]):
    rgbObj={}
    rgbObj['r']=colorExporter[i,0] #organizar matriz a exportar por color rojo
    rgbObj['g']=colorExporter[i,1] #organizar matriz a exportar por color verde
    rgbObj['b']=colorExporter[i,2] #organizar matriz a exportar por color azul
    color['items'].append(rgbObj)

l.write2JSONFile(path,'colors',color) # exportar matriz de color con extension .json
np.savetxt('J_energy.txt', J_energy, fmt='%.2e') # exportar matriz de J con extension .txt
